...
const dict = {
  de: {
    ...
    hero: {
      ...
      title1: "Echter Döner. Echte Pizza.",
      title2: "Richtig Gut.",
...
const App = () => {
  ...
  const HomeFAQSection = () => {
    ...
    return (
      <section className="py-10 md:py-16 max-w-3xl mx-auto w-full mt-4">
        <div className="text-center mb-8">
          <p className="text-[11px] md:text-xs font-bold tracking-[0.25em] uppercase text-[#B26941] mb-2">{t.homeFaq.eyebrow}</p>
          <h2 className="font-outfit text-3xl md:text-[3.5rem] font-semibold tracking-[-0.04em] text-[#111111] leading-[1.1]">
            {t.homeFaq.title}
          </h2>
        </div>

        <div className="bg-[#F2F1EC] rounded-3xl md:rounded-[2rem] overflow-hidden text-left border border-[#E8E7E2]">
          {faqsList.map((faq, index) => (
            ...
            >
              <button 
                ...
                className="w-full flex justify-between items-center p-5 md:p-8 hover:bg-[#ebeae5] transition-colors active:bg-[#e4e3de]"
              >
                <span className="font-outfit text-base md:text-xl font-medium pr-4 text-left">{faq.q}</span>
              ...
              >
                <div className="overflow-hidden">
                  <div className="px-5 md:px-8 pb-6 md:pb-8 text-[#5A5A5A] text-sm md:text-base font-light leading-relaxed pr-10 md:pr-20 whitespace-pre-line">
                    {faq.a}
                  </div>
                </div>
              </div>
  ...
  const HomePage = () => (
    <>
      {/* SECTION 1 - HERO */}
      <section className="pt-6 md:pt-8 pb-12 md:pb-20 flex flex-col items-center text-center">
        <p className="text-[11px] md:text-xs font-bold tracking-[0.25em] uppercase text-[#B26941] mb-4">
          {t.hero.eyebrow}
        </p>
        <h1 className="font-outfit text-4xl sm:text-5xl md:text-5xl lg:text-6xl font-medium leading-[1.05] tracking-[-0.03em] mb-4 md:mb-6 max-w-4xl mx-auto text-[#111111]">
          {t.hero.title1} <br className="hidden sm:block" /> {t.hero.title2}
        </h1>
        <p className="text-[#5A5A5A] text-sm sm:text-base md:text-lg lg:text-xl max-w-xl mx-auto mb-6 md:mb-8 leading-relaxed font-light">
          {t.hero.subtitle}
        </p>
        <div className="flex flex-col sm:flex-row justify-center items-stretch sm:items-center gap-3 md:gap-4 mb-6 w-full px-4 sm:px-0 max-w-md mx-auto sm:max-w-none">
          <a href="https://www.lieferando.de" target="_blank" rel="noreferrer" className="w-full sm:w-auto text-center bg-[#FFCBA4] hover:bg-[#ffbd8f] text-[#1A1A1A] px-5 md:px-8 py-3.5 md:py-3.5 rounded-full font-medium text-base transition-all active:scale-95 shadow-sm whitespace-nowrap">
            {t.hero.btnPrimary}
          </a>
          <button onClick={() => navigateTo('menu')} className="w-full sm:w-auto text-center bg-transparent border border-[#E8E7E2] hover:border-[#B26941] text-[#B26941] px-5 md:px-8 py-3.5 md:py-3.5 rounded-full font-medium text-base transition-all active:scale-95 whitespace-nowrap">
            {t.hero.btnSecondary}
          </button>
        </div>
        <p className="text-[#7A7A7A] text-xs md:text-sm font-medium tracking-wide mb-8 md:mb-10 px-4">
          {t.hero.hours}
        </p>
        <div className="w-full">
          <div className="w-full aspect-[4/3] md:aspect-[21/9] rounded-3xl md:rounded-[3rem] overflow-hidden group">
      ...
      <section className="my-10 md:my-16 bg-[#F2F1EC] rounded-3xl md:rounded-[3rem] px-5 md:px-12 py-10 md:py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
          <div className="flex flex-col items-center text-center md:items-start md:text-left">
            <div className="w-12 h-12 md:w-14 md:h-14 bg-[#1A1A1A] text-[#FFCBA4] rounded-full flex items-center justify-center mb-4 md:mb-5">
              <Utensils className="w-5 h-5 md:w-6 md:h-6" />
            </div>
            <h3 className="font-outfit text-lg md:text-xl font-medium text-[#1A1A1A] mb-2 md:mb-3">{t.whatWeDo.kebabTitle}</h3>
          ...
          <div className="flex flex-col items-center text-center md:items-start md:text-left">
            ...
            <h3 className="font-outfit text-lg md:text-xl font-medium text-[#1A1A1A] mb-2 md:mb-3">{t.whatWeDo.pizzaTitle}</h3>
            <p className="text-[#5A5A5A] text-base font-light leading-relaxed">
          ...
          <div className="flex flex-col items-center text-center md:items-start md:text-left">
            ...
            <h3 className="font-outfit text-lg md:text-xl font-medium text-[#1A1A1A] mb-2 md:mb-3">{t.whatWeDo.saladsTitle}</h3>
            <p className="text-[#5A5A5A] text-base font-light leading-relaxed">
      ...
      <section className="py-10 md:py-16 border-t border-[#F0F0F0]">
        <div className="max-w-4xl mx-auto flex flex-col md:flex-row gap-8 md:gap-10 items-center">
          <div className="flex-1 text-center md:text-left">
            <h2 className="font-outfit text-3xl md:text-[3.5rem] font-semibold tracking-[-0.04em] mb-4 md:mb-6 leading-[1.1] text-[#111111]">
              {t.aboutSnippet.title1} <br className="hidden md:block"/> {t.aboutSnippet.title2}
            </h2>
            <p className="text-[#5A5A5A] text-base md:text-lg mb-3 md:mb-4 font-light leading-relaxed">
              {t.aboutSnippet.p1}
            </p>
            <p className="text-[#5A5A5A] text-base md:text-lg mb-6 md:mb-8 font-light leading-relaxed">
              {t.aboutSnippet.p2}
            </p>
            <button onClick={() => navigateTo('about')} className="text-[#B26941] text-base font-medium border-b border-[#B26941] pb-1 hover:text-[#1A1A1A] hover:border-[#1A1A1A] transition-colors inline-block">
              {t.aboutSnippet.btn}
            </button>
          </div>
          <div className="flex-1 w-full aspect-square md:aspect-auto md:h-[400px] rounded-3xl md:rounded-[2.5rem] overflow-hidden">
      ...
      <section className="py-10 md:py-16 border-t border-[#F0F0F0]">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 md:mb-8 gap-4">
          <div>
            <p className="text-[11px] md:text-xs font-bold tracking-[0.25em] uppercase text-[#B26941] mb-2">{t.featured.eyebrow}</p>
            <h2 className="font-outfit text-3xl md:text-[3.5rem] font-semibold tracking-[-0.04em] text-[#111111] leading-[1.1]">
              {t.featured.title}
            </h2>
          </div>
          <button onClick={() => navigateTo('menu')} className="text-[#B26941] text-base font-medium border-b border-[#B26941] pb-1 hover:text-[#1A1A1A] hover:border-[#1A1A1A] transition-colors mt-1 md:mt-0">
            {t.featured.btn}
          </button>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
          <div className="bg-white rounded-3xl md:rounded-[2rem] border border-[#F0F0F0] overflow-hidden hover:shadow-md transition-shadow group flex flex-col">
            ...
            <div className="p-5 md:p-8 flex flex-col flex-grow">
              <div className="flex justify-between items-start mb-2 md:mb-3">
                <h3 className="font-outfit text-xl md:text-2xl font-medium text-[#1A1A1A]">Iskender Kebap</h3>
                <span className="font-medium text-[#1A1A1A] text-base md:text-lg">14.00 €</span>
              </div>
              <p className="text-[#5A5A5A] text-base font-light leading-relaxed mb-4 flex-grow">{t.featured.item1Desc}</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl md:rounded-[2rem] border border-[#F0F0F0] overflow-hidden hover:shadow-md transition-shadow group flex flex-col">
            ...
            <div className="p-5 md:p-8 flex flex-col flex-grow">
              <div className="flex justify-between items-start mb-2 md:mb-3">
                <h3 className="font-outfit text-xl md:text-2xl font-medium text-[#1A1A1A]">Pizza Star</h3>
                <span className="font-medium text-[#1A1A1A] text-base md:text-lg">12.00 €</span>
              </div>
              <p className="text-[#5A5A5A] text-base font-light leading-relaxed mb-4 flex-grow">{t.featured.item2Desc}</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl md:rounded-[2rem] border border-[#F0F0F0] overflow-hidden hover:shadow-md transition-shadow group flex flex-col">
            ...
            <div className="p-5 md:p-8 flex flex-col flex-grow">
              <div className="flex justify-between items-start mb-2 md:mb-3">
                <h3 className="font-outfit text-xl md:text-2xl font-medium text-[#1A1A1A]">Döner Überbacken</h3>
                <span className="font-medium text-[#1A1A1A] text-base md:text-lg">13.00 €</span>
              </div>
              <p className="text-[#5A5A5A] text-base font-light leading-relaxed mb-4 flex-grow">{t.featured.item3Desc}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-10 md:py-16 border-t border-[#F0F0F0] overflow-hidden">
        <div className="flex flex-col items-center justify-center text-center mb-8 md:mb-10">
          <div className="flex gap-1 text-[#FFCBA4] mb-2 md:mb-3">
          ...
          <h2 className="font-outfit text-3xl md:text-[3.5rem] font-semibold tracking-[-0.04em] mb-2 md:mb-3 text-[#111111]">
            {t.reviews.title}
          </h2>
          <p className="text-[#5A5A5A] text-base md:text-lg font-light">
            {t.reviews.subtitle}
          </p>
        </div>

        <div className="flex overflow-x-auto gap-3 md:gap-6 pb-6 md:pb-8 hide-scrollbar snap-x snap-mandatory px-4 md:px-0">
          {reviews.map((review) => (
            <div 
              key={review.id} 
              className="snap-start shrink-0 w-[260px] md:w-[350px] bg-white rounded-3xl md:rounded-[2rem] p-5 md:p-8 border border-[#F0F0F0] shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow"
            >
              <div>
                <div className="flex gap-1 mb-4 md:mb-5 text-[#FFCBA4]">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} fill={i < review.rating ? "currentColor" : "none"} className="w-4 h-4 md:w-4 md:h-4" />
                  ))}
                </div>
                <p className="text-[#1A1A1A] font-light leading-relaxed mb-4 md:mb-6 text-base md:text-lg">"{review.text}"</p>
              </div>
              <div>
                {review.image && (
                  <div className="w-full h-32 md:h-36 rounded-xl overflow-hidden mb-4 md:mb-5">
                    <img src={review.image} className="w-full h-full object-cover" alt="Review photo" />
                  </div>
                )}
                <div className="flex justify-between items-center text-sm border-t border-[#F0F0F0] pt-3 md:pt-4">
      ...
      <section className="py-10 md:py-24 border-t border-[#F0F0F0]">
        <div className="flex flex-col items-center text-center mb-10 md:mb-16">
          <p className="text-[11px] md:text-xs font-bold tracking-[0.25em] uppercase text-[#B26941] mb-2 md:mb-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 md:gap-8">
          ...
          <div className="bg-white p-6 md:p-10 rounded-3xl md:rounded-[2.5rem] border border-[#E8E7E2] text-center shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-300 flex flex-col items-center group">
            <div className="w-16 h-16 md:w-20 md:h-20 bg-[#F9F8F6] group-hover:bg-[#FFCBA4] text-[#B26941] group-hover:text-[#1A1A1A] rounded-full flex items-center justify-center mb-5 md:mb-6 transition-colors duration-300">
              <Users className="w-7 h-7 md:w-8 md:h-8" />
            </div>
            <h3 className="font-outfit text-xl md:text-2xl font-medium text-[#1A1A1A] mb-3 md:mb-4">{t.orderOptions.dineInTitle}</h3>
            <p className="text-[#5A5A5A] text-base font-light leading-relaxed mb-6 md:mb-8 flex-grow">{t.orderOptions.dineInDesc}</p>
            <button onClick={() => navigateTo('reservation')} className="text-[#1A1A1A] text-sm font-medium border-b-2 border-[#E8E7E2] hover:border-[#B26941] pb-1 transition-colors uppercase tracking-wide">
              {t.orderOptions.dineInBtn}
            </button>
          </div>
          
          {/* CARD 2: TAKEAWAY (Soft Beige) */}
          <div className="bg-[#F2F1EC] p-6 md:p-10 rounded-3xl md:rounded-[2.5rem] border border-transparent text-center hover:shadow-xl hover:-translate-y-2 transition-all duration-300 flex flex-col items-center group">
            <div className="w-16 h-16 md:w-20 md:h-20 bg-white group-hover:bg-[#FFCBA4] text-[#B26941] group-hover:text-[#1A1A1A] rounded-full flex items-center justify-center mb-5 md:mb-6 transition-colors duration-300 shadow-sm">
              <ShoppingBag className="w-7 h-7 md:w-8 md:h-8" />
            </div>
            <h3 className="font-outfit text-xl md:text-2xl font-medium text-[#1A1A1A] mb-3 md:mb-4">{t.orderOptions.takeawayTitle}</h3>
            <p className="text-[#5A5A5A] text-base font-light leading-relaxed mb-6 md:mb-8 flex-grow">{t.orderOptions.takeawayDesc}</p>
            <a href="tel:+4970239424183" className="text-[#1A1A1A] text-sm font-medium border-b-2 border-[#dcdad1] hover:border-[#B26941] pb-1 transition-colors uppercase tracking-wide">
              {t.orderOptions.takeawayBtn}
            </a>
          </div>

          {/* CARD 3: DELIVERY (Dark Charcoal Accent) */}
          <div className="bg-[#1A1A1A] p-6 md:p-10 rounded-3xl md:rounded-[2.5rem] border border-[#1A1A1A] text-center shadow-xl hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 flex flex-col items-center relative overflow-hidden group">
            <div className="absolute -top-12 -right-12 w-40 h-40 md:w-48 md:h-48 bg-[#B26941] rounded-full blur-[60px] md:blur-[70px] opacity-20 group-hover:opacity-40 transition-opacity duration-500"></div>
            
            <div className="w-16 h-16 md:w-20 md:h-20 bg-[#2A2A2A] text-[#FFCBA4] rounded-full flex items-center justify-center mb-5 md:mb-6 shadow-inner relative z-10 transition-transform duration-300 group-hover:scale-110">
              <Truck className="w-7 h-7 md:w-8 md:h-8" />
            </div>
            <h3 className="font-outfit text-xl md:text-2xl font-medium text-white mb-3 md:mb-4 relative z-10">{t.orderOptions.deliveryTitle}</h3>
            <p className="text-[#A0A0A0] text-base font-light leading-relaxed mb-6 md:mb-8 flex-grow relative z-10">{t.orderOptions.deliveryDesc}</p>
            <a href="https://www.lieferando.de" target="_blank" rel="noreferrer" className="w-full text-center bg-[#FFCBA4] hover:bg-white text-[#1A1A1A] py-3.5 md:py-4 rounded-full text-base font-semibold transition-colors relative z-10 tracking-wide active:scale-95 shadow-md">
  ...
  const MenuPage = () => {
    ...
    return (
      <section className="py-8 md:py-16 max-w-6xl mx-auto">
        
        <div className="text-center mb-8 md:mb-10">
          <p className="text-[11px] md:text-xs font-bold tracking-[0.25em] uppercase text-[#B26941] mb-2 md:mb-4">{t.menu.eyebrow}</p>
          <h1 className="font-outfit text-4xl md:text-6xl font-medium tracking-[-0.03em] text-[#111111]">{t.menu.title}</h1>
        </div>

        {/* --- CATEGORY OVERVIEW (HORIZONTAL SCROLL ON MOBILE) --- */}
        <div className="mb-8 md:mb-12">
          <div className="flex items-center gap-2 mb-4 md:mb-6 px-2">
            <h2 className="font-outfit text-xl md:text-2xl font-medium text-[#1A1A1A]">{t.menu.categories}</h2>
            <ChevronDown size={18} className="text-[#B26941]" />
          </div>
          
          <div className="flex md:grid md:grid-cols-3 lg:grid-cols-4 overflow-x-auto gap-2 md:gap-4 pb-4 md:pb-0 hide-scrollbar -mx-4 px-4 md:mx-0 md:px-0">
            {Object.keys(menuData).map((cat) => (
              <button 
                key={cat}
                onClick={() => setActiveCategory(cat)}
                className={`shrink-0 p-3 md:p-6 rounded-2xl md:rounded-[1.5rem] border text-center transition-all active:scale-95 flex items-center justify-center min-h-[50px] md:min-h-[80px] ${
                  activeCategory === cat 
                    ? 'bg-[#FFCBA4] border-[#FFCBA4] text-[#1A1A1A] shadow-sm' 
                    : 'bg-white border-[#E8E7E2] text-[#5A5A5A] hover:border-[#B26941] hover:text-[#1A1A1A]'
                }`}
              >
                <h3 className="font-outfit text-sm md:text-lg font-medium tracking-tight whitespace-nowrap md:whitespace-normal px-2 md:px-0">{cat}</h3>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl md:rounded-[2rem] p-5 md:p-10 shadow-sm border border-[#F0F0F0]">
          <div className="flex justify-between items-end border-b border-[#F0F0F0] pb-4 md:pb-6 mb-6 md:mb-8">
            <h2 className="font-outfit text-2xl md:text-4xl font-semibold text-[#1A1A1A]">{activeCategory}</h2>
            <span className="text-[#7A7A7A] text-xs md:text-sm font-medium">{menuData[activeCategory]?.length || 0} {t.menu.items}</span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6 md:gap-y-10">
            {menuData[activeCategory] && menuData[activeCategory].map((item, index) => (
              <div key={index} className="flex justify-between items-start gap-3 md:gap-4 group">
                <div className="flex-1">
                  <h3 className="font-outfit text-lg md:text-xl font-medium text-[#1A1A1A] group-hover:text-[#B26941] transition-colors">{item.name}</h3>
                  {item.desc && (
                    <p className="text-[#7A7A7A] text-sm md:text-base mt-1 md:mt-1.5 font-light leading-relaxed pr-4">{item.desc}</p>
                  )}
                </div>
                <div className="flex flex-col items-end gap-1.5 md:gap-2 shrink-0">
                  ...
                  {item.isVeg && (
                    <div className="w-6 h-6 md:w-7 md:h-7 rounded-full bg-[#E8F3EA] text-[#4CAF50] flex items-center justify-center" title="Vegetarian">
                      <Leaf size={12} className="md:w-[14px] md:h-[14px]" />
                    </div>
                  )}
                </div>
              </div>
  ...
  const AboutPage = () => (
    <section className="py-8 md:py-16 max-w-4xl mx-auto">
      <p className="text-[11px] md:text-xs font-bold tracking-[0.25em] uppercase text-[#B26941] mb-3 md:mb-4 text-center">{t.aboutFull.eyebrow}</p>
      <h1 className="font-outfit text-4xl md:text-6xl font-medium tracking-[-0.03em] mb-8 md:mb-10 text-[#111111] text-center">{t.aboutFull.title}</h1>
      
      <div className="w-full aspect-[16/9] rounded-3xl md:rounded-[2.5rem] overflow-hidden mb-8 md:mb-12 shadow-sm">
        <img src="1000178837.jpg" alt="Restaurant Interior" className="w-full h-full object-cover" />
      </div>

      <div className="prose prose-base md:prose-lg text-[#5A5A5A] font-light leading-relaxed max-w-none space-y-8 md:space-y-12">
        
        {/* Section 1 */}
        <div>
          <h2 className="text-xl md:text-3xl text-[#1A1A1A] font-medium leading-snug mb-3 md:mb-4">
            {t.aboutFull.intro1}
          </h2>
          <p className="text-base md:text-lg">{t.aboutFull.intro2}</p>
        </div>
        ...
        <div>
          ...
          <p className="text-base md:text-lg mb-3 md:mb-4">{t.aboutFull.section2p1}</p>
          <p className="text-base md:text-lg">{t.aboutFull.section2p2}</p>
        </div>
        ...
        <div>
          ...
          <p className="text-base md:text-lg mb-3 md:mb-4">{t.aboutFull.section3p1}</p>
          <p className="text-base md:text-lg mb-3 md:mb-4">{t.aboutFull.section3p2}</p>
          <p className="text-base md:text-lg mb-3 md:mb-4">{t.aboutFull.section3p3}</p>
          <p className="text-base md:text-lg">{t.aboutFull.section3p4}</p>
        </div>

        {/* Section 4 */}
        <div>
          <h2 className="text-xl md:text-3xl text-[#1A1A1A] font-medium leading-snug mb-4 md:mb-6">
            {t.aboutFull.howToGet}
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 pt-1 md:pt-2">
            <div className="bg-white p-5 md:p-8 rounded-3xl md:rounded-[2rem] border border-[#E8E7E2] text-center">
              <div className="w-12 h-12 md:w-14 md:h-14 bg-[#F9F8F6] text-[#B26941] rounded-full flex items-center justify-center mx-auto mb-3 md:mb-4"><Utensils size={20} className="md:w-6 md:h-6"/></div>
              <h3 className="font-outfit text-lg md:text-xl font-medium text-[#1A1A1A] mb-2 md:mb-3">{t.orderOptions.dineInTitle}</h3>
              <p className="text-base text-[#5A5A5A]">{t.aboutFull.dineInDesc}</p>
            </div>
            <div className="bg-white p-5 md:p-8 rounded-3xl md:rounded-[2rem] border border-[#E8E7E2] text-center">
              <div className="w-12 h-12 md:w-14 md:h-14 bg-[#F9F8F6] text-[#B26941] rounded-full flex items-center justify-center mx-auto mb-3 md:mb-4"><ShoppingBag size={20} className="md:w-6 md:h-6"/></div>
              <h3 className="font-outfit text-lg md:text-xl font-medium text-[#1A1A1A] mb-2 md:mb-3">{t.orderOptions.takeawayTitle}</h3>
              <p className="text-base text-[#5A5A5A]">{t.aboutFull.takeawayDesc}</p>
            </div>
            <div className="bg-white p-5 md:p-8 rounded-3xl md:rounded-[2rem] border border-[#E8E7E2] text-center">
              <div className="w-12 h-12 md:w-14 md:h-14 bg-[#F9F8F6] text-[#B26941] rounded-full flex items-center justify-center mx-auto mb-3 md:mb-4"><Truck size={20} className="md:w-6 md:h-6"/></div>
              <h3 className="font-outfit text-lg md:text-xl font-medium text-[#1A1A1A] mb-2 md:mb-3">{t.orderOptions.deliveryTitle}</h3>
              <p className="text-base text-[#5A5A5A]">{t.aboutFull.deliveryDesc}</p>
            </div>
          </div>
        </div>

        {/* Hours & Contact Block (Pre-Map) */}
        <div className="bg-[#F2F1EC] p-6 md:p-10 rounded-3xl md:rounded-[2.5rem] mt-8 md:mt-12 text-center border border-[#E8E7E2]">
          <h2 className="font-outfit text-xl md:text-3xl text-[#1A1A1A] font-medium mb-4 md:mb-6">{t.aboutFull.hoursLocationTitle}</h2>
          <div className="flex flex-col md:flex-row justify-center items-center gap-4 md:gap-12 mb-6 md:mb-8 text-[#5A5A5A] text-base md:text-lg">
            <div>
          ...
          <div className="flex flex-col sm:flex-row justify-center items-center gap-3 md:gap-4 w-full">
            <button onClick={() => navigateTo('menu')} className="bg-white border border-[#E8E7E2] hover:border-[#B26941] text-[#B26941] px-5 md:px-8 py-3.5 md:py-3.5 rounded-full font-medium text-base transition-all active:scale-95 whitespace-nowrap w-full sm:w-auto">
              {t.hero.btnSecondary}
            </button>
            <a href="https://www.lieferando.de" target="_blank" rel="noreferrer" className="bg-[#FFCBA4] hover:bg-[#ffbd8f] text-[#1A1A1A] px-5 md:px-8 py-3.5 md:py-3.5 rounded-full font-medium text-base transition-all active:scale-95 shadow-sm whitespace-nowrap w-full sm:w-auto">
              {t.hero.btnPrimary}
            </a>
          </div>
        </div>
  ...
  const ContactPage = () => (
    <section className="py-8 md:py-16 max-w-6xl mx-auto">
      <p className="text-[11px] md:text-xs font-bold tracking-[0.25em] uppercase text-[#B26941] mb-3 md:mb-4 text-center">{t.contact.eyebrow}</p>
      <h1 className="font-outfit text-4xl md:text-6xl font-medium tracking-[-0.03em] mb-8 md:mb-10 text-[#111111] text-center">{t.contact.title}</h1>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-start">
        
        {/* Contact Details & FAQ snippet */}
        <div className="space-y-6 md:space-y-8">
          <div className="bg-white p-6 md:p-8 rounded-3xl md:rounded-[2.5rem] shadow-sm border border-[#E8E7E2] space-y-5 md:space-y-8">
            <div className="flex items-start gap-3 md:gap-4">
              <MapPin className="text-[#B26941] shrink-0 mt-1 w-5 h-5 md:w-6 md:h-6" />
              <div>
                <h3 className="font-outfit font-medium text-lg md:text-xl text-[#1A1A1A] mb-1">{t.contact.location}</h3>
                <p className="text-[#5A5A5A] text-base font-light">Brunnenstraße 1<br/>73235 Weilheim an der Teck<br/>Germany</p>
              </div>
            </div>
            <div className="flex items-start gap-3 md:gap-4">
              <Phone className="text-[#B26941] shrink-0 mt-1 w-5 h-5 md:w-6 md:h-6" />
              <div>
                <h3 className="font-outfit font-medium text-lg md:text-xl text-[#1A1A1A] mb-1">{t.contact.phone}</h3>
                <a href="tel:+4970239424183" className="text-[#5A5A5A] text-base font-light hover:text-[#B26941] transition-colors">+49 7023 9424183</a>
              </div>
            </div>
            <div className="flex items-start gap-3 md:gap-4">
              <Clock className="text-[#B26941] shrink-0 mt-1 w-5 h-5 md:w-6 md:h-6" />
              <div>
                <h3 className="font-outfit font-medium text-lg md:text-xl text-[#1A1A1A] mb-1">{t.contact.hours}</h3>
                <p className="text-[#5A5A5A] text-base font-light whitespace-pre-line">{t.contact.hoursText}</p>
              </div>
            </div>
          </div>

          <div className="bg-[#F2F1EC] p-6 md:p-8 rounded-3xl md:rounded-[2.5rem] border border-[#E8E7E2]">
            <h3 className="font-outfit font-medium text-xl md:text-2xl text-[#1A1A1A] mb-4 md:mb-5">{t.contact.faqTitle}</h3>
            <div className="space-y-4 md:space-y-5">
              {faqs.slice(0,2).map((faq, i) => (
                <div key={i}>
                  <h4 className="font-medium text-[#1A1A1A] text-base mb-1">{faq.question}</h4>
                  <p className="text-[#5A5A5A] font-light text-sm md:text-base leading-relaxed">{faq.answer}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Contact Form */}
        <div className="bg-white p-6 md:p-8 rounded-3xl md:rounded-[2.5rem] shadow-sm border border-[#E8E7E2]">
          <h2 className="font-outfit text-2xl md:text-3xl font-medium mb-5 md:mb-6 text-[#1A1A1A]">{t.contact.formTitle}</h2>
          <form className="space-y-4 md:space-y-5" onSubmit={(e) => e.preventDefault()}>
            <div>
              <label className="block text-sm font-medium text-[#7A7A7A] mb-1.5 px-2">{t.contact.lblName}</label>
              {/* Added text-base to prevent iOS Safari auto-zoom on input focus */}
              <input type="text" className="w-full bg-[#F9F8F6] border border-[#E8E7E2] rounded-xl md:rounded-2xl px-4 md:px-5 py-3.5 text-base outline-none focus:border-[#B26941] focus:bg-white transition-colors" placeholder={t.contact.plName} />
            </div>
            <div>
              <label className="block text-sm font-medium text-[#7A7A7A] mb-1.5 px-2">{t.contact.lblEmail}</label>
              <input type="email" className="w-full bg-[#F9F8F6] border border-[#E8E7E2] rounded-xl md:rounded-2xl px-4 md:px-5 py-3.5 text-base outline-none focus:border-[#B26941] focus:bg-white transition-colors" placeholder={t.contact.plEmail} />
            </div>
            <div>
              <label className="block text-sm font-medium text-[#7A7A7A] mb-1.5 px-2">{t.contact.lblMessage}</label>
              <textarea rows="4" className="w-full bg-[#F9F8F6] border border-[#E8E7E2] rounded-2xl md:rounded-3xl px-4 md:px-5 py-3.5 text-base outline-none focus:border-[#B26941] focus:bg-white transition-colors resize-none" placeholder={t.contact.plMessage}></textarea>
            </div>
            <button className="w-full bg-[#FFCBA4] hover:bg-[#ffbd8f] text-[#1A1A1A] px-5 md:px-8 py-3.5 md:py-4 rounded-full font-medium text-base md:text-lg transition-all active:scale-95 mt-1 md:mt-2">
              {t.contact.btnSubmit}
            </button>
          </form>
        </div>
  ...
  const ReservationPage = () => (
    <section className="py-8 md:py-16 max-w-3xl mx-auto">
      <p className="text-[11px] md:text-xs font-bold tracking-[0.25em] uppercase text-[#B26941] mb-3 md:mb-4 text-center">{t.reservation.eyebrow}</p>
      <h1 className="font-outfit text-4xl md:text-6xl font-medium tracking-[-0.03em] mb-3 md:mb-4 text-[#111111] text-center">{t.reservation.title}</h1>
      <p className="text-[#5A5A5A] text-base md:text-lg mb-6 md:mb-8 font-light text-center max-w-lg mx-auto">
      ...
      <div className="bg-white p-6 md:p-8 rounded-3xl md:rounded-[2.5rem] shadow-sm border border-[#E8E7E2]">
        <form className="space-y-4 md:space-y-5" onSubmit={(e) => e.preventDefault()}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
            <div>
              <label className="block text-sm font-medium text-[#7A7A7A] mb-1.5 px-2">{t.reservation.lblDate}</label>
              <div className="relative">
                <Calendar className="absolute left-3 md:left-4 top-1/2 -translate-y-1/2 text-[#A0A0A0]" size={18} />
                <input type="date" className="w-full bg-[#F9F8F6] border border-[#E8E7E2] rounded-xl md:rounded-2xl pl-10 md:pl-11 pr-4 md:pr-5 py-3.5 text-base outline-none focus:border-[#B26941] focus:bg-white transition-colors text-[#1A1A1A]" />
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium text-[#7A7A7A] mb-1.5 px-2">{t.reservation.lblTime}</label>
              <div className="relative">
                <Clock className="absolute left-3 md:left-4 top-1/2 -translate-y-1/2 text-[#A0A0A0]" size={18} />
                <input type="time" className="w-full bg-[#F9F8F6] border border-[#E8E7E2] rounded-xl md:rounded-2xl pl-10 md:pl-11 pr-4 md:pr-5 py-3.5 text-base outline-none focus:border-[#B26941] focus:bg-white transition-colors text-[#1A1A1A]" />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
            <div>
              <label className="block text-sm font-medium text-[#7A7A7A] mb-1.5 px-2">{t.reservation.lblGuests}</label>
              <div className="relative">
                <Users className="absolute left-3 md:left-4 top-1/2 -translate-y-1/2 text-[#A0A0A0]" size={18} />
                <select className="w-full bg-[#F9F8F6] border border-[#E8E7E2] rounded-xl md:rounded-2xl pl-10 md:pl-11 pr-4 md:pr-5 py-3.5 text-base outline-none focus:border-[#B26941] focus:bg-white transition-colors text-[#1A1A1A] appearance-none">
                  {[1,2,3,4,5,6,7,8,"8+"].map(n => <option key={n} value={n}>{n} {n === 1 ? t.reservation.optPerson : t.reservation.optPeople}</option>)}
                </select>
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium text-[#7A7A7A] mb-1.5 px-2">{t.reservation.lblName}</label>
              <input type="text" className="w-full bg-[#F9F8F6] border border-[#E8E7E2] rounded-xl md:rounded-2xl px-4 md:px-5 py-3.5 text-base outline-none focus:border-[#B26941] focus:bg-white transition-colors text-[#1A1A1A]" />
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
            <div>
              <label className="block text-sm font-medium text-[#7A7A7A] mb-1.5 px-2">{t.reservation.lblPhone}</label>
              <input type="tel" className="w-full bg-[#F9F8F6] border border-[#E8E7E2] rounded-xl md:rounded-2xl px-4 md:px-5 py-3.5 text-base outline-none focus:border-[#B26941] focus:bg-white transition-colors text-[#1A1A1A]" placeholder="+49 " />
            </div>
            <div>
              <label className="block text-sm font-medium text-[#7A7A7A] mb-1.5 px-2">{t.reservation.lblEmail}</label>
              <input type="email" className="w-full bg-[#F9F8F6] border border-[#E8E7E2] rounded-xl md:rounded-2xl px-4 md:px-5 py-3.5 text-base outline-none focus:border-[#B26941] focus:bg-white transition-colors text-[#1A1A1A]" placeholder="" />
            </div>
          </div>

          <div>
            <label className="block text-sm font-medium text-[#7A7A7A] mb-1.5 px-2">{t.reservation.lblRequests}</label>
            <textarea rows="3" className="w-full bg-[#F9F8F6] border border-[#E8E7E2] rounded-2xl md:rounded-3xl px-4 md:px-5 py-3.5 text-base outline-none focus:border-[#B26941] focus:bg-white transition-colors resize-none text-[#1A1A1A]" placeholder={t.reservation.plRequests}></textarea>
          </div>

          <button className="w-full bg-[#1A1A1A] hover:bg-black text-white px-5 md:px-8 py-3.5 md:py-4 rounded-full font-medium text-base md:text-lg transition-all active:scale-95 mt-1 md:mt-2 shadow-md">
            {t.reservation.btnSubmit}
          </button>
          <p className="text-center text-[10px] md:text-xs text-[#7A7A7A] mt-2 md:mt-3">{t.reservation.note}</p>
        </form>
      </div>
    </section>
  );
